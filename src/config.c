...
#include<ctype.h>
   ...
   //工具函数：去掉字符串首尾空白
   //C标准库没有 trim 函数，嵌入式经常要自己写
   static char* trim(char*s){
    //跳过开头的空白，
    while(*s&&isspace((unsigned char)*s))
    s++;
    if(*s=='\0') return s;//全市空白
    //从末尾删除空白
    ...
    *(end+1)='\0';
    ...
    //解析 "key = value" 一行
   //返回: 0=成功  -1=跳过
    static int parse_line(const char*line,char *key,int key_size,char *val,int val_size){
        const char *eq=strchr(line,'=');
        if(!eq) return -1;//没有=跳过
        /* 提取 key */
        size_t klen=eq-line;// 指针相减 = 偏移量
        if(klen>=(size_t)key_size)
            klen=key_size-1;
        memcpy(key,line,klen);
        key[klen]='\0';
        char*k=trim(key);// key.trim()
        memmove(key,k,strlen(k)+1);//原地搬
        //提取
        strncpy(val,eq+1,val_size-1);
        val[val_size-1]='\0';
        char *v=trim(val);
        memmove(val,v,strlen(v)+1);
        return 0;
    }
    ...
    if (!fp) {
        printf("[config] ⚠️  %s not found, using defaults\n", path);
        return -1;
    }
    ...
    while (fgets(line, sizeof(line), fp)) {
        ...
        if (parse_line(line, key, sizeof(key), val, sizeof(val)) < 0) {
            printf("[config] ⚠️  line %d: cannot parse, skipped\n", lineno);
            continue;
        }
        ...
        if (strcmp(key, "server.port") == 0) {
            ...
            if (sscanf(field, "[%d].%31s%n", &idx, rest, &pos) >= 2 && pos > 0) {
                if (idx < 0 || idx >= MAX_SENSORS) {
                    printf("[config] ⚠️  line %d: sensor[%d] out of range\n",
                           lineno, idx);
                    continue;
                }
                if (strcmp(rest, "name") == 0)
                    snprintf(cfg->sensors[idx].name, sizeof(cfg->sensors[idx].name),
                             "%.31s", val);
                    ...
                    printf("[config] ⚠️  line %d: unknown field '%s'\n",
                           lineno, rest);

                if (idx + 1 > cfg->sensor_count)
                    cfg->sensor_count = idx + 1;
            } else {
                printf("[config] ⚠️  line %d: bad key '%s'\n", lineno, key);
            }
        }
        else {
            printf("[config] ⚠️  line %d: unknown key '%s'\n", lineno, key);
        }
    }
